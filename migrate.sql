...
INSERT INTO team_members (member_id, user_id, team_id, nickname)
  SELECT
    worker_id,
    user_id,
    command_id,
...
INSERT INTO ch_text
  SELECT DISTINCT
    ...
    old_id
  FROM chapters c
    INNER JOIN ruranobe_db.mw_page p ON page_title = replace(c.url, '/v1/', '/')
    INNER JOIN ruranobe_db.mw_revision rv ON rev_page = page_id
    INNER JOIN ruranobe_db.mw_text t ON old_id = rev_text_id
  WHERE rev_timestamp = (SELECT max(rev_timestamp)
                         FROM ruranobe_db.mw_revision
...
Кстати, если вы хотите сотрудничать с нами на уровне проекта, то можете связаться по <a href="https://ruranobe.ru/contact">почте</a>.
==Помощь защитная==
...
INSERT INTO chapters (volume_id, url, title) VALUES
  (@diary_volume_id, 'system/diary/01012016', '1 января 2016'),
  (@diary_volume_id, 'system/diary/28122015', '28 декабря 2015'),
...
INSERT INTO texts
SET
  text_wiki = 'Дорогой дневник, вот и наступил новый год. Дождались все-таки. К моему удивлению, даже выпало немного снега, да еще и минус на улице был. Хоть в небольшой степени, но зима все же пришла на праздник. И этому я очень рада!\nХочется поздравить моих ребят и всех тех, кто поддерживал их. С Новым Годом! Мы преодолели много трудностей и продолжим их преодолевать. Я верю, что мы совместными силами справимся со всем!\nВот прошел очередной год, а я не знаю, повзрослела ли. Нет, телом-то я разумеется выросла (вроде), а вот в остальном плане… Но мне хочется верить, что да!\nА теперь время продолжать праздновать! Я решила повеселиться в этот раз по-полной, а то все в заботах да в заботах. Ну и за Валерой надо присмотреть, а то излишние возлияния не прошли даром, устроит еще чего (снова!).\nИ еще раз, с Новым Годом!';
UPDATE chapters
SET text_id = last_insert_id()
WHERE url = 'system/diary/01012016';

INSERT INTO texts
SET
  text_wiki = 'Дорогой дневник, что-то мне хочется взять что-нибудь потяжелее и устроить дома разборки. А все почему? А потому ребятам, похоже, никак не найти место для дел, кроме как у меня. Я понимаю, что сейчас надо много делать, но почему у меня-то?! (」゜ロ゜)」 Я словно в сумасшедшем доме нахожусь. Мало того, что все носятся как угорелые, так еще и меня подпрячь пытаются, хотя я и так занята. Хорошо, что я привыкла уже к подобному. Но все же тишину мне ждать лишь по ночам (но и тут у меня есть сомнения, с них-то станется).\nКстати, надеюсь, всем понравился мой образ Снегурочки! Все опять устроил Валера, кто ж еще. Я до сих пор поражаюсь, откуда он все это берет? И ведь на вопросы не отвечает, гад, только хитро улыбается! Но ничего, я таки у него все выведаю! Хе-хе-хе. (￢‿￢)\nА тем временем меня продолжает «радовать» погода. На улице плюс, зеленая травка… Канун Нового Года, как-никак!\nА наши редакторы и не думают заканчивать со своей вакханалией. Более того, они еще и других утянули! Даже не представляю, что они после всего этого выдадут. Поживем – увидим…';
UPDATE chapters
SET text_id = last_insert_id()
WHERE url = 'system/diary/28122015';
